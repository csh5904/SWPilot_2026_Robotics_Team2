...
def generate_launch_description():
    ...
    # 6. 로봇을 원 운동 시키는 파이썬 제어 노드 (circle_drive_node.py)
    node_circle_drive = Node(
        package='my_robot_sim',
        executable='circle_drive_node',  # setup.py의 entry_points에 등록한 이름과 일치 필수
        name='circle_drive_node',
        output='screen'
    )
    ...
    return LaunchDescription([
        ...
        node_circle_drive            # 4순위: 출현한 로봇에게 cmd_vel 속도 명령을 쏨
    ])