ROS 2에서 파이썬을 이용해 기본 노드를 작성하고, 로깅(Logging) 메커니즘을 활용하며, `setup.py` 등록 및 빌드/실행에 이르는 전 과정을 마크다운 문서로 완벽히 정리했습니다. 과제 제출 및 학습용 파일로 바로 활용할 수 있는 구조입니다.
...
로봇 시스템은 디스플레이 화면이 없는 경우가 많으므로 일반 `print()` 대신 레벨별 **로깅(Logging) 메서드**를 사용합니다. `Node` 클래스는 자체적으로 `get_logger()` 객체를 내장하고 있어 아래와 같이 세분화된 로그를 발생시킵니다.
...
1. **터미널 콘솔 출력:** 노드가 실행 중인 터미널 창에 실시간으로 시간(Timestamp), 로그 레벨, 노드 이름과 함께 텍스트 포맷으로 즉시 출력됩니다.
2. **rqt_console 활용:** GUI 기반 모니터링 툴인 `ros2 run rqt_console rqt_console` 명령어를 실행하면, 노드들이 뿜어내는 로그를 레벨별/노드별로 필터링하여 직관적인 그래픽 인터페이스로 검출할 수 있습니다.
3. **로컬 로그 파일 시스템:** 빌드 시 자동으로 워크스페이스 루트 아래에 생성된 `~/.ros/log` 또는 `~/ros2_ws/log/` 디렉토리에 텍스트와 데이터 스트림 파일(`.log`) 형태로 날짜별로 영구 기록됩니다.
...
### 4.2 빌드 방법 및 `--symlink-install` 옵션의 절대적 역할

워크스페이스 루트로 이동해 전용 빌드 명령어를 실행합니다. 파이썬 코드 개발 시 이 옵션은 매우 핵심적입니다.
...
* **`--symlink-install` 옵션의 역할:** 기본 빌드를 수행하면 파이썬 코드가 `install` 폴더로 통째로 복사됩니다. 이 경우 소스 코드를 단 한 줄만 수정해도 매번 `colcon build`를 새로 쳐야 바뀐 코드가 적용됩니다. 하지만 `--symlink-install`을 사용하면 실제 소스 파일과 `install` 디렉토리 사이에 심볼릭 링크(바로가기 개념)가 생성됩니다. 따라서 **빌드를 한 번만 해두면 이후 `src` 내의 파이썬 코드를 수정하고 저장하는 즉시 재빌드 없이 변경 사항이 런타임에 즉각 반영**됩니다.
