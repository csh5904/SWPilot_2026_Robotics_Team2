ROS 2의 타이머 메커니즘과 비동기 프로그래밍의 핵심인 콜백(Callback) 함수의 개념을 정리하고, 요구된 조건(2초 주기 증가, 3초 주기 감소 타이머 듀얼 구동)을 완벽히 만족하는 예제 코드와 상세 분석을 마크다운 문서로 작성했습니다.

이전 단계에서 겪었던 중복 셧다운 에러를 방지하는 방어 코드(`rclpy.ok()`)까지 깔끔하게 녹여냈으니 그대로 과제 파일로 활용하면 됩니다.
